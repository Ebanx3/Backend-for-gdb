...
const login = async (req: Request, res: Response) => {
    try {
        ...
        const response: any = await userConnection.getElement("users", {
    ...
    try {
        ...
        if (!password || !email)
            return res
                ...
                .json({
                    success: false,
                    message: "Must have password and email fields",
                });

        const response = await userConnection.getElement("users", {
            email: email.toLowerCase(),
        });
        if (response)
            return res
        ...
        userConnection.addELement("users", {
            email: email.toLowerCase(),
...
const isValidToken = async (req: Request, res: Response) => {
    try {
        ...
        if (!tokenAuth)
            return res
                .status(400)
                .json({ success: false, message: "Headers must have a token" });
